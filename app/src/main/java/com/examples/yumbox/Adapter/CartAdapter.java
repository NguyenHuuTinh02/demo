...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHolder> {
    ...
    private ArrayList<CartItem> menuItems;
    ...
    public class CartViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(int position) {
            ...
            binding.cartItemPrice.setText(FormatString.formatAmount(Integer.parseInt(cartItem.getFoodPrice())));
    ...
    private void getUniqueKey() {
        ...
        cartItemRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Log.d("DX05782", "onCancelled: " + error.getMessage());
            }
        });
    ...
    public void removeItemAtPosition(int position) {
        if (position < 0 || position >= itemQuantities.length) {
            Log.d("DX05782", "removeItemAtPosition: " + position);
            return;
        }
    ...
    private void initTable() {
        NameTableAdapter.tableNameResult(row -> {
            if (!row) {
                if (!menuItems.isEmpty() && itemQuantities[10] > 3) {
                    menuItems.add(cartItems.get(menuItems.size()));
                    cartItems = menuItems;
                    LpmAdapter.newRow();
                }
            }
